...
- **Data Strategy** — a pipeline architecture diagram, the real-vs-illustrative badge-discipline
  ...
  **Live Integrity Gate** — 8 reconciliation checks that run in your browser on every page load,
  not just in a Node test file someone would have to clone the repo to see.
...
1. **`node verify.cjs`** — an independent Node-based tie-out. Stubs a real (not no-op) `classList`,
   ...
   **both branches of every alert/verdict function**, including the branches this build's own
   ...
   tab-rail status pill's real visible/hidden/text/class state.
   **131 assertions, all passing**
   (`node verify.cjs | grep -c "^pass:"` → 131) as of the last run. Exists because this repo was
